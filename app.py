...
model_choice = st.selectbox("Model", ["ARIMA", "NeuralProphet", "LSTM", "Hybrid (base NP/ARIMA + LSTM residual)"])
    ...
    with st.spinner("Running model... this may take time depending on choice"):
        try:
            if model_choice == "ARIMA":
                ...
                metrics = compute_metrics(y_test, pred)
                st.write("Metrics:", metrics)
                st.pyplot(plot_series(idx_test, y_test, idx_test, pred, title="NeuralProphet Forecast"))
                ...
                # choose base: try NeuralProphet then ARIMA
                base_name = st.radio("Hybrid base model", options=["NeuralProphet","ARIMA"], index=0)
                if base_name == "NeuralProphet":
                    try:
                        m, idx_b, y_b, base_pred = fit_neuralprophet_and_predict(df, test_h=test_h, epochs=40)
                    except Exception as e:
                        st.error(f"NeuralProphet base failed: {e}")
                        base_pred = None
                else:
                    try:
                        exog = df[['temp','solar','wind']].copy() if set(['temp','solar','wind']).issubset(df.columns) else None
